...
BIN_PATH="${1:-$ROOT_DIR/target/debug/R.A.I.N.}"
TMP_ROOT="/tmp/R.A.I.N.-tmux-onboard-$$"
...
if [[ ! -x "$BIN_PATH" ]]; then
  echo "Building R.A.I.N...."
  cargo build --bin R.A.I.N. >/dev/null
fi
...
run_update_custom_model_flow() {
  ...
  env R.A.I.N._CONFIG_DIR="$config_dir" \
