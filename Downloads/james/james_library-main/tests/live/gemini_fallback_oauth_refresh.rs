...
//!
//! Requires:
//! - Live Gemini OAuth profile in `~/.R.A.I.N./auth-profiles.json` with refresh_token
...
async fn gemini_warmup_refreshes_expired_oauth_token() -> Result<()> {
    ...
    if !auth_profiles_path.exists() {
        ...
        eprintln!("Run: R.A.I.N. auth login --provider gemini");
        return Ok(());
    }
    ...
    let gemini_profile_key = profiles
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "No Gemini OAuth profile found. Run: R.A.I.N. auth login --provider gemini"
            )
        })?
        .clone();
