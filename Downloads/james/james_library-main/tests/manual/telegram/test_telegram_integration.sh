#!/bin/bash
# R.A.I.N. Telegram Integration Test Suite
...
if [ -f "target/release/R.A.I.N." ]; then
    BINARY_SIZE=$(ls -lh target/release/R.A.I.N. | awk '{print $5}')
    SIZE_BYTES=$(stat -f%z target/release/R.A.I.N. 2>/dev/null || stat -c%s target/release/R.A.I.N.)
...
CONFIG_PATH="$HOME/.R.A.I.N./config.toml"
if [ -f "$CONFIG_PATH" ]; then
    ...
    if grep -q "\[channels_config.telegram\]" "$CONFIG_PATH"; then
        ...
        warn "Telegram not configured - run 'R.A.I.N. onboard' first"
    fi
else
    warn "No config file found - run 'R.A.I.N. onboard' first"
fi
...
START_TIME=$(date +%s)
HEALTH_OUTPUT=$(timeout 10 target/release/R.A.I.N. channel doctor 2>&1 || true)
END_TIME=$(date +%s)
