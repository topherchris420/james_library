# Triển khai mạng — R.A.I.N. trên Raspberry Pi và mạng nội bộ

Tài liệu này hướng dẫn triển khai R.A.I.N. trên Raspberry Pi hoặc host khác trong mạng nội bộ, với các channel Telegram và webhook tùy chọn.
...
|------|----------------------|----------|
| **Telegram polling** | Không | R.A.I.N. poll Telegram API; hoạt động từ bất kỳ đâu |
| **Matrix sync (kể cả E2EE)** | Không | R.A.I.N. sync qua Matrix client API; không cần webhook đến |
...
**Lưu ý:** Telegram, Discord và Slack dùng **long-polling** — R.A.I.N. thực hiện các request ra ngoài. Không cần port forwarding hoặc public IP.

---

## 2. R.A.I.N. trên Raspberry Pi
...
- Channel Telegram hoạt động: R.A.I.N. poll Telegram API (outbound)
...
```bash
R.A.I.N. daemon --host 0.0.0.0 --port 3000
```
...
2. Khởi động tunnel:
   ...
   ```
   Hoặc dùng `R.A.I.N. tunnel` (xem tài liệu tunnel).

3. R.A.I.N. sẽ từ chối `0.0.0.0` trừ khi `allow_public_bind = true` hoặc có tunnel đang hoạt động.
...
- R.A.I.N. gọi `https://api.telegram.org/bot{token}/getUpdates`
...
Chạy `R.A.I.N. daemon` — channel Telegram khởi động tự động.
...
- Chỉ chạy một instance runtime cho cùng token (khuyến nghị: service `R.A.I.N. daemon`).
...
- [ ] Chạy `R.A.I.N. daemon --host 127.0.0.1 --port 3000` (Telegram hoạt động không cần 0.0.0.0)
