# R.A.I.N. trên Nucleo-F401RE — Hướng dẫn từng bước

Chạy R.A.I.N. trên Mac hoặc Linux. Kết nối Nucleo-F401RE qua USB. Điều khiển GPIO (LED, các pin) qua Telegram hoặc CLI.

---

## Lấy thông tin board qua Telegram (Không cần nạp firmware)

R.A.I.N. có thể đọc thông tin chip từ Nucleo qua USB **mà không cần nạp firmware nào**. Nhắn tin cho Telegram bot của bạn:
...
R.A.I.N. bao gồm mọi thứ cần thiết cho Nucleo-F401RE:
...
| Flash command | `R.A.I.N. peripheral flash-nucleo` | Build firmware, nạp qua probe-rs |
...
Từ thư mục gốc của repo R.A.I.N.:
...
```bash
R.A.I.N. agent --message "Turn on the LED on pin 13"
```
...
| 3 | `R.A.I.N. peripheral flash-nucleo` |
| 4 | Thêm Nucleo vào config.toml (path = serial port của bạn) |
| 5 | `R.A.I.N. daemon` hoặc `R.A.I.N. agent -m "Turn on LED"` |
...
- **Lệnh GPIO bị bỏ qua** — Kiểm tra `path` trong config có khớp với serial port của bạn. Chạy `R.A.I.N. peripheral list` để xác nhận.
