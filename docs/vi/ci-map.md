...
- `.github/workflows/pub-docker-img.yml` (`Docker`)
    - Mục đích: kiểm tra Docker smoke trên PR lên `main` và publish image khi push tag (`v*`) only
...
- `Docker`: push tag (`v*`) để publish, PR lên `main` tương ứng để smoke build, dispatch thủ công chỉ smoke
...
- `Dependabot`: tất cả PR cập nhật nhắm vào `main`
