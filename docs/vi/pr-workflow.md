# Quy trình PR R.A.I.N. (Cộng tác khối lượng cao)

Tài liệu này định nghĩa cách R.A.I.N. xử lý khối lượng PR lớn trong khi vẫn duy trì:
...
- Tất cả PR của contributor nhắm trực tiếp vào `main`.
...
- PR ảnh hưởng Rust sử dụng cùng bộ gate bắt buộc như push lên `main` (không có shortcut chỉ build trên PR).
...
- Nếu có ngôn ngữ giống danh tính trong test/ví dụ, cần được chuẩn hóa về nhãn gốc R.A.I.N./dự án.
...
- Tự động hóa `pr-hygiene` kiểm tra các PR mở mỗi 12 giờ và đăng nhắc nhở khi PR không có commit mới trong 48+ giờ và rơi vào một trong hai trường hợp: đang tụt hậu so với `main` hoặc thiếu/thất bại `CI Required Gate` trên head commit.
...
- Xác nhận bất kỳ ngôn ngữ giống danh tính nào đều sử dụng thuật ngữ gốc R.A.I.N./dự án.
