...
1. Biến `R.A.I.N._WORKSPACE` (nếu được đặt)
2. Marker `~/.R.A.I.N./active_workspace.toml` (nếu có)
3. Mặc định `~/.R.A.I.N./config.toml`

R.A.I.N. ghi log đường dẫn config đã giải quyết khi khởi động ở mức `INFO`:

- `Config loaded` với các trường: `path`, `workspace`, `source`, `initialized`

Lệnh xuất schema:

- `R.A.I.N. config schema` (xuất JSON Schema draft 2020-12 ra stdout)
...
| `otel_service_name` | `R.A.I.N.` | Tên dịch vụ gửi đến OTLP collector |
...
1. `R.A.I.N._PROVIDER` (ghi đè tường minh, luôn thắng khi có giá trị)
...
- Dùng `R.A.I.N._PROVIDER` khi cố ý muốn biến môi trường ghi đè provider đã cấu hình.
...
- Mặc định an toàn: R.A.I.N. **không** clone hay đồng bộ `open-skills` trừ khi `open_skills_enabled = true`.
- Ghi đè qua biến môi trường:
  - `R.A.I.N._OPEN_SKILLS_ENABLED` chấp nhận `1/0`, `true/false`, `yes/no`, `on/off`.
  - `R.A.I.N._OPEN_SKILLS_DIR` ghi đè đường dẫn kho khi có giá trị.
- Thứ tự ưu tiên: `R.A.I.N._OPEN_SKILLS_ENABLED` → `skills.open_skills_enabled` trong `config.toml` → mặc định `false`.
...
- R.A.I.N. yêu cầu Composio v3 tools với `toolkit_versions=latest` và thực thi với `version="latest"` để tránh bản tool mặc định cũ.
...
| `user_id` | `R.A.I.N.` | `MEM0_USER_ID` | User ID để phân vùng memory |
| `app_name` | `R.A.I.N.` | `MEM0_APP_NAME` | Tên ứng dụng đăng ký trong mem0 |
...
3. Kiểm tra bằng `R.A.I.N. doctor` trước khi khởi động lại/triển khai.
...
- Khi `R.A.I.N. channel start` đang chạy, thay đổi `default_provider`, `default_model`, `default_temperature`, `api_key`, `api_url` và `reliability.*` được áp dụng nóng từ `config.toml` ở tin nhắn tiếp theo.
