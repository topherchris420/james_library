# R.A.I.N. trên Arduino Uno Q — Hướng dẫn từng bước

Chạy R.A.I.N. trên phía Linux của Arduino Uno Q. Telegram hoạt động qua WiFi; điều khiển GPIO dùng Bridge (yêu cầu một ứng dụng App Lab tối giản).

---

## Những gì đã có sẵn (Không cần thay đổi code)

R.A.I.N. bao gồm mọi thứ cần thiết cho Arduino Uno Q. **Clone repo và làm theo hướng dẫn này — không cần patch hay code tùy chỉnh nào.**
...
| Setup command | `src/peripherals/uno_q_setup.rs` | `R.A.I.N. peripheral setup-uno-q` triển khai Bridge qua scp + arduino-app-cli |
...
## Phase 2: Cài đặt R.A.I.N. trên Uno Q
...
# Clone R.A.I.N. (hoặc scp project của bạn)
...
# Cài đặt
sudo cp target/release/R.A.I.N. /usr/local/bin/
```
...
# Copy sang Uno Q
scp target/aarch64-unknown-linux-gnu/release/R.A.I.N. arduino@<UNO_Q_IP>:~/
ssh arduino@<UNO_Q_IP> "sudo mv ~/R.A.I.N. /usr/local/bin/"
```
...
# Cấu hình nhanh
R.A.I.N. onboard --api-key YOUR_OPENROUTER_KEY --provider openrouter

# Hoặc tạo config thủ công
mkdir -p ~/.R.A.I.N./workspace
nano ~/.R.A.I.N./config.toml
```
...
## Phase 4: Chạy R.A.I.N. Daemon

```bash
ssh arduino@<UNO_Q_IP>

# Chạy daemon (Telegram polling hoạt động qua WiFi)
R.A.I.N. daemon --host 127.0.0.1 --port 3000
```

**Tại bước này:** Telegram chat hoạt động. Gửi tin nhắn tới bot — R.A.I.N. phản hồi. Chưa có GPIO.

---

## Phase 5: GPIO qua Bridge (R.A.I.N. xử lý tự động)

R.A.I.N. bao gồm Bridge app và setup command.

### 5.1 Triển khai Bridge App

**Từ Mac** (với repo R.A.I.N.):
```bash
R.A.I.N. peripheral setup-uno-q --host 192.168.0.48
```

**Từ Uno Q** (đã SSH vào):
```bash
R.A.I.N. peripheral setup-uno-q
```
...
Giờ khi bạn nhắn tin cho Telegram bot *"Turn on the LED"* hoặc *"Set pin 13 high"*, R.A.I.N. dùng `gpio_write` qua Bridge.
...
| 5 | `git clone https://github.com/R.A.I.N.-labs/R.A.I.N..git && cd R.A.I.N.` |
| 6 | `cargo build --release --no-default-features` |
| 7 | `R.A.I.N. onboard --api-key KEY --provider openrouter` |
| 8 | Chỉnh sửa `~/.R.A.I.N./config.toml` (thêm Telegram bot_token) |
| 9 | `R.A.I.N. daemon --host 127.0.0.1 --port 3000` |
...
- **"command not found: R.A.I.N."** — Dùng đường dẫn đầy đủ: `/usr/local/bin/R.A.I.N.` hoặc đảm bảo `~/.cargo/bin` nằm trong PATH.
...
- **Lệnh GPIO bị bỏ qua** — Đảm bảo Bridge app đang chạy (`R.A.I.N. peripheral setup-uno-q` triển khai và khởi động nó). Config phải có `board = "arduino-uno-q"` và `transport = "bridge"`.
- **LLM provider (GLM/Zhipu)** — Dùng `default_provider = "glm"` hoặc `"zhipu"` với `GLM_API_KEY` trong env hoặc config. R.A.I.N. dùng endpoint v4 chính xác.
