# R.A.I.N. PR 工作流（高协作吞吐量场景）

本文档定义了 R.A.I.N. 在高 PR 提交量场景下的处理规则，以保持：
...
- 影响 Rust 代码的 PR 使用与 `main` 推送相同的必需检查集（无 PR 专属构建快捷方式）。
...
- 如果测试/示例中出现类似身份的措辞，已标准化为 R.A.I.N./项目原生标签。
...
- `pr-hygiene` 自动化每 12 小时检查开放 PR，当 PR 48 小时以上无新提交且落后于 `main` 或头部提交的 `CI Required Gate` 缺失/失败时，发布提醒。
...
- 确认任何必需的类似身份措辞使用了 R.A.I.N./项目原生术语。
