...
本指南介绍 R.A.I.N. 的原生 Nextcloud Talk 集成。
...
- 设置 `R.A.I.N._NEXTCLOUD_TALK_WEBHOOK_SECRET` 时会覆盖 `webhook_secret`。
...
配置 `webhook_secret` 时，R.A.I.N. 会验证：
...
- R.A.I.N. 忽略来自机器人的 webhook 事件（`actorType = bots`）。
- R.A.I.N. 忽略非消息/系统事件。
