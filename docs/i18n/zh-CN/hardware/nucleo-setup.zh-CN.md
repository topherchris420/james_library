# Nucleo-F401RE 上的 R.A.I.N. — 分步指南

在 Mac 或 Linux 主机上运行 R.A.I.N.。通过 USB 连接 Nucleo-F401RE。通过 Telegram 或 CLI 控制 GPIO（LED、引脚）。

---

## 通过 Telegram 获取开发板信息（无需固件）

R.A.I.N. 可以通过 USB 从 Nucleo 读取芯片信息，**无需烧录任何固件**。向你的 Telegram 机器人发送消息：
...
R.A.I.N. 包含 Nucleo-F401RE 所需的一切：
...
| 烧录命令 | `R.A.I.N. peripheral flash-nucleo` | 构建固件，通过 probe-rs 烧录 |
...
| 5 | `R.A.I.N. daemon` 或 `R.A.I.N. agent -m "Turn on LED"` |
...
- **GPIO 命令被忽略** — 检查配置中的 `path` 与你的串口匹配。运行 `R.A.I.N. peripheral list` 验证。
