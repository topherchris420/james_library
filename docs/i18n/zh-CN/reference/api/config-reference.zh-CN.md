...
1. `R.A.I.N._WORKSPACE` 覆盖（如果设置）
2. 持久化的 `~/.R.A.I.N./active_workspace.toml` 标记（如果存在）
3. 默认 `~/.R.A.I.N./config.toml`

R.A.I.N. 在启动时以 `INFO` 级别记录解析后的配置：

- `Config loaded` 包含字段：`path`、`workspace`、`source`、`initialized`

模式导出命令：

- `R.A.I.N. config schema`（将 JSON Schema 草案 2020-12 打印到 stdout）
...
| `otel_service_name` | `R.A.I.N.` | 发送到 OTLP 收集器的服务名称 |
...
1. `R.A.I.N._PROVIDER`（显式覆盖，非空时始终优先）
...
- 当你有意让运行时环境覆盖非默认配置的提供商时，请使用 `R.A.I.N._PROVIDER`。
...
- 当 `enabled = true` 且不存在 OTP 密钥时，R.A.I.N. 会生成一个并打印一次注册 URI。
...
| `state_file` | `~/.R.A.I.N./estop-state.json` | 持久化 estop 状态路径 |
...
- 使用 CLI 命令 `R.A.I.N. estop` 启动，`R.A.I.N. estop resume` 清除级别。
...
- 安全优先默认：除非 `open_skills_enabled = true`，否则 R.A.I.N. **不会**克隆或同步 `open-skills`。
- 环境覆盖：
  - `R.A.I.N._OPEN_SKILLS_ENABLED` 接受 `1/0`、`true/false`、`yes/no`、`on/off`。
  - `R.A.I.N._OPEN_SKILLS_DIR` 非空时覆盖仓库路径。
  - `R.A.I.N._SKILLS_PROMPT_MODE` 接受 `full` 或 `compact`。
- 启用标志的优先级：`R.A.I.N._OPEN_SKILLS_ENABLED` → `config.toml` 中的 `skills.open_skills_enabled` → 默认 `false`。
...
- R.A.I.N. 请求 Composio v3 工具时使用 `toolkit_versions=latest`，并使用 `version=\"latest\"` 执行工具，以避免过时的默认工具版本。
...
| `user_id` | `R.A.I.N.` | `MEM0_USER_ID` | 记忆作用域的用户 ID |
| `app_name` | `R.A.I.N.` | `MEM0_APP_NAME` | 在 mem0 中注册的应用名称 |
...
- 当 `R.A.I.N. channel start` 运行时，`default_provider`、`default_model`、`default_temperature`、`api_key`、`api_url` 和 `reliability.*` 的更新会在下一条入站消息时从 `config.toml` 热应用。
...
- 设置时 `R.A.I.N._LINQ_SIGNING_SECRET` 覆盖 `signing_secret`。
...
- 设置时 `R.A.I.N._NEXTCLOUD_TALK_WEBHOOK_SECRET` 覆盖 `webhook_secret`。
