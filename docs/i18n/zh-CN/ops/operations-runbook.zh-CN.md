...
| 模式 | 命令 | 使用场景 |
|---|---|---|
| 前台运行时 | `R.A.I.N. daemon` | 本地调试、短期会话 |
| 仅前台网关 | `R.A.I.N. gateway` | webhook 端点测试 |
| 用户服务 | `R.A.I.N. service install && R.A.I.N. service start` | 持久化运维管理的运行时 |
...
| 信号 | 命令 / 文件 | 预期结果 |
|---|---|---|
| 配置有效性 | `R.A.I.N. doctor` | 无严重错误 |
| 渠道连通性 | `R.A.I.N. channel doctor` | 配置的渠道健康 |
| 运行时摘要 | `R.A.I.N. status` | 预期的提供商/模型/渠道 |
| 守护进程心跳/状态 | `~/.R.A.I.N./daemon_state.json` | 文件定期更新 |
...
4. 如果渠道仍然失败，验证 `~/.R.A.I.N./config.toml` 中的白名单和凭证。
