# R.A.I.N. on Arduino Uno Q — Step-by-Step Guide

Run R.A.I.N. on the Arduino Uno Q's Linux side. Telegram works over WiFi; GPIO control uses the Bridge (requires a minimal App Lab app).

---

## What's Included (No Code Changes Needed)

R.A.I.N. includes everything needed for Arduino Uno Q. **Clone the repo and follow this guide — no patches or custom code required.**
...
| Setup command | `src/peripherals/uno_q_setup.rs` | `R.A.I.N. peripheral setup-uno-q` deploys the Bridge via scp + arduino-app-cli |
...
## Phase 2: Install R.A.I.N. on Uno Q
...
# Clone R.A.I.N. (or scp your project)
...
**At this point:** Telegram chat works. Send messages to your bot — R.A.I.N. responds. No GPIO yet.

---

## Phase 5: GPIO via Bridge (R.A.I.N. Handles It)

R.A.I.N. includes the Bridge app and setup command.

### 5.1 Deploy Bridge App

**From your Mac** (with R.A.I.N. repo):
```bash
...
Now when you message your Telegram bot *"Turn on the LED"* or *"Set pin 13 high"*, R.A.I.N. uses `gpio_write` via the Bridge.
...
| 8 | Edit `~/.R.A.I.N./config.toml` (add Telegram bot_token) |
| 9 | `R.A.I.N. daemon --host 127.0.0.1 --port 42617` |
...
- **"command not found: R.A.I.N."** — Use full path: `/usr/local/bin/R.A.I.N.` or ensure `~/.cargo/bin` is in PATH.
...
- **GPIO commands ignored** — Ensure Bridge app is running (`R.A.I.N. peripheral setup-uno-q` deploys and starts it). Config must have `board = "arduino-uno-q"` and `transport = "bridge"`.
- **LLM provider (GLM/Zhipu)** — Use `default_provider = "glm"` or `"zhipu"` with `GLM_API_KEY` in env or config. R.A.I.N. uses the correct v4 endpoint.
