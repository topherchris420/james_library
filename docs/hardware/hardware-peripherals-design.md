# Hardware Peripherals Design — R.A.I.N.

R.A.I.N. enables microcontrollers (MCUs) and Single Board Computers (SBCs) to **dynamically interpret natural language commands**, generate hardware-specific code, and execute peripheral interactions in real-time.

## 1. Vision

**Goal:** R.A.I.N. acts as a hardware-aware AI agent that:
...
**Mental model:** R.A.I.N. = brain that understands hardware. Peripherals = arms and legs it controls.
...
R.A.I.N. runs **directly on the device**. The board spins up a gRPC/nanoRPC server and communicates with peripherals locally.
...
2. R.A.I.N. fetches board-specific docs (e.g. ESP32 GPIO mapping)
...
R.A.I.N. runs on the **host** and maintains a hardware-aware link to the target. Used for development, introspection, and flashing.
...
2. R.A.I.N. identifies connected hardware (VID/PID, architecture)
...
2. R.A.I.N. writes/flashes via OpenOCD or probe-rs
3. Confirms success

**Or:**
1. R.A.I.N. auto-discovers: *"STM32 Nucleo on /dev/ttyACM0, ARM Cortex-M4"*
...
|------------------|--------------------------------|----------------------------------|
| R.A.I.N. runs on | Device (ESP32, RPi)           | Host (Mac, Linux)                |
...
Host runs R.A.I.N.; peripheral runs minimal firmware. Simple JSON over serial.

### Mode B: RPi as Host (Native GPIO)

R.A.I.N. on Pi; GPIO via rppal or sysfs. No separate firmware.
...
1. **Startup:** R.A.I.N. loads config, sees `peripherals.boards`.
...
For low-latency, typed RPC between R.A.I.N. and peripherals:
...
- **R.A.I.N.-firmware** or **R.A.I.N.-peripheral** — a separate crate/workspace.
...
- User flashes this to the board; R.A.I.N. connects and discovers capabilities.

## 9. Implementation Phases

### Phase 1: Skeleton ✅ (Done)

- [x] Add `Peripheral` trait, config schema, CLI (`R.A.I.N. peripheral list/add`)
- [x] Add `--peripheral` flag to agent
- [x] Document in AGENTS.md

### Phase 2: Host-Mediated — Hardware Discovery ✅ (Done)

- [x] `R.A.I.N. hardware discover`: enumerate USB devices (VID/PID)
- [x] Board registry: map VID/PID → architecture, name (e.g. Nucleo-F401RE)
- [x] `R.A.I.N. hardware introspect <path>`: memory map, peripheral list
...
- [x] R.A.I.N. on Raspberry Pi (native GPIO via rppal)
...
- [ ] R.A.I.N. *on* ESP32 (WiFi + LLM, edge-native) — future
...
- Running full R.A.I.N. *on* bare STM32 (no WiFi, limited RAM) — use Host-Mediated instead
...
> *"Boards like ESP, Raspberry Pi, or boards with WiFi can connect to an LLM (Gemini or open-source). R.A.I.N. runs on the device, creates its own gRPC, spins it up, and communicates with peripherals. User asks via WhatsApp: 'move X arm' or 'turn on LED'. R.A.I.N. gets accurate documentation, writes code, executes it, stores it optimally, runs it, and turns on the LED — all on the development board.*
>
> *For STM Nucleo connected via USB/J-Link/Aardvark to my Mac: R.A.I.N. from my Mac accesses the hardware, installs or writes what it wants on the device, and returns the result. Example: 'Hey R.A.I.N., what are the available/readable addresses on this USB device?' It can figure out what's connected where and suggest."*
