# Network Deployment — R.A.I.N. on Raspberry Pi and Local Network

This document covers deploying R.A.I.N. on a Raspberry Pi or other host on your local network, with Telegram and optional webhook channels.
...
|------|----------------------|----------|
| **Telegram polling** | No | R.A.I.N. polls Telegram API; works from anywhere |
| **Matrix sync (including E2EE)** | No | R.A.I.N. syncs via Matrix client API; no inbound webhook required |
...
**Key:** Telegram, Discord, Slack, and Nostr use **outbound connections** — R.A.I.N. connects to external servers/relays. No port forwarding or public IP required.

---

## 2. R.A.I.N. on Raspberry Pi
...
- Telegram channel works: R.A.I.N. polls Telegram API (outbound)
...
```bash
R.A.I.N. daemon --host 0.0.0.0 --port 42617
```
...
2. Start a tunnel:
   ...
   ```
   Or use `R.A.I.N. tunnel` (see tunnel docs).

3. R.A.I.N. will refuse `0.0.0.0` unless `allow_public_bind = true` or a tunnel is active.
...
- R.A.I.N. calls `https://api.telegram.org/bot{token}/getUpdates`
...
Run `R.A.I.N. daemon` — Telegram channel starts automatically.
...
- Keep one runtime instance for the same token (recommended: `R.A.I.N. daemon` service).
...
- [ ] Run `R.A.I.N. daemon --host 127.0.0.1 --port 42617` (Telegram works without 0.0.0.0)
...
R.A.I.N. supports OpenRC for Alpine Linux and other distributions using the OpenRC init system. OpenRC services run **system-wide** and require root/sudo.

### 7.1 Prerequisites

- Alpine Linux (or another OpenRC-based distro)
- Root or sudo access
- A dedicated `R.A.I.N.` system user (created during install)

### 7.2 Install Service

```bash
# Install service (OpenRC is auto-detected on Alpine)
sudo R.A.I.N. service install
```

This creates:
- Init script: `/etc/init.d/R.A.I.N.`
- Config directory: `/etc/R.A.I.N./`
- Log directory: `/var/log/R.A.I.N./`

### 7.3 Configuration

Manual config copy is usually not required.

`sudo R.A.I.N. service install` automatically prepares `/etc/R.A.I.N.`, migrates existing runtime state from your user setup when available, and sets ownership/permissions for the `R.A.I.N.` service user.

If no prior runtime state is available to migrate, create `/etc/R.A.I.N./config.toml` before starting the service.

### 7.4 Enable and Start

```bash
# Add to default runlevel
sudo rc-update add R.A.I.N. default

# Start the service
sudo rc-service R.A.I.N. start

# Check status
sudo rc-service R.A.I.N. status
```
...
| Command | Description |
|---------|-------------|
| `sudo rc-service R.A.I.N. start` | Start the daemon |
| `sudo rc-service R.A.I.N. stop` | Stop the daemon |
| `sudo rc-service R.A.I.N. status` | Check service status |
| `sudo rc-service R.A.I.N. restart` | Restart the daemon |
| `sudo R.A.I.N. service status` | R.A.I.N. status wrapper (uses `/etc/R.A.I.N.` config) |
...
| Log | Path |
|-----|------|
| Access/stdout | `/var/log/R.A.I.N./access.log` |
| Errors/stderr | `/var/log/R.A.I.N./error.log` |

View logs:

```bash
sudo tail -f /var/log/R.A.I.N./error.log
```
...
sudo rc-service R.A.I.N. stop
sudo rc-update del R.A.I.N. default

# Remove init script
sudo R.A.I.N. service uninstall
```
...
- The service runs as the `R.A.I.N.:R.A.I.N.` user (least privilege)
- Config must be at `/etc/R.A.I.N./config.toml` (explicit path in init script)
- If the `R.A.I.N.` user does not exist, install will fail with instructions to create it

### 7.9 Checklist: Alpine/OpenRC Deployment

- [ ] Install: `sudo R.A.I.N. service install`
- [ ] Enable: `sudo rc-update add R.A.I.N. default`
- [ ] Start: `sudo rc-service R.A.I.N. start`
- [ ] Verify: `sudo rc-service R.A.I.N. status`
- [ ] Check logs: `/var/log/R.A.I.N./error.log`
