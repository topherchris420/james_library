...
- CLI `R.A.I.N. sop` currently manages definitions only: `list`, `validate`, `show`.
...
2. Create a SOP directory, for example:
   ...
   ```text
   ~/.R.A.I.N./workspace/sops/deploy-prod/SOP.toml
   ~/.R.A.I.N./workspace/sops/deploy-prod/SOP.md
   ```
...
3. Validate and inspect definitions:
   ...
   ```bash
   R.A.I.N. sop list
   R.A.I.N. sop validate
   R.A.I.N. sop show deploy-prod
   ```
