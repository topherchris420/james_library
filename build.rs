...
fn main() {
    ...
    println!("cargo:rerun-if-changed=docs/assets/R.A.I.N.-trans.png");
...
fn ensure_dashboard_assets(dist_dir: &Path) {
    ...
    let src = Path::new("docs/assets/R.A.I.N.-trans.png");
    if !src.exists() {
        eprintln!(
            "cargo:warning=docs/assets/R.A.I.N.-trans.png not found; skipping dashboard asset copy"
        );
        return;
    }

    let dst = dist_dir.join("R.A.I.N.-trans.png");
    if let Err(e) = fs::copy(src, &dst) {
        eprintln!("cargo:warning=Failed to copy R.A.I.N.-trans.png into web/dist/: {e}");
    }
}
