...
fn show_integration_info(config: &Config, name: &str) -> Result<()> {
    ...
    let Some(entry) = entries.iter().find(|e| e.name.to_lowercase() == name_lower) else {
        anyhow::bail!(
            "Unknown integration: {name}. Check README for supported integrations or run `R.A.I.N. onboard` to configure channels/providers."
        );
    };
    ...
    match entry.name {
        "Telegram" => {
            ...
            println!("    3. Run: R.A.I.N. onboard --channels-only");
            println!("    4. Start: R.A.I.N. channel start");
        }
        "Discord" => {
            println!("  Setup:");
            ...
            println!("    4. Run: R.A.I.N. onboard --channels-only");
        }
        "Slack" => {
            println!("  Setup:");
        ...
        "Browser" => {
            println!("  Built-in:");
            println!("    R.A.I.N. can control Chrome/Chromium for web tasks.");
            println!("    Uses headless browser automation.");
        }
        "Cron" => {
            println!("  Built-in:");
            println!("    Schedule tasks in ~/.R.A.I.N./workspace/cron/");
            println!("    Run: R.A.I.N. cron list");
        }
        "Weather" => {
            println!("  Built-in:");
        ...
        "Webhooks" => {
            ...
            println!("    Run: R.A.I.N. gateway");
        }
        _ => {
            if status == IntegrationStatus::ComingSoon {
                println!("  This integration is planned. Stay tuned!");
                println!("  Track progress: https://github.com/R.A.I.N.-labs/R.A.I.N.");
            }
        }
    }
