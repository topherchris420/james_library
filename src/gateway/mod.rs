...
/// Read gateway request timeout from `R.A.I.N._GATEWAY_TIMEOUT_SECS` env var
...
pub fn gateway_request_timeout_secs() -> u64 {
    std::env::var("R.A.I.N._GATEWAY_TIMEOUT_SECS")
        .ok()
...
pub async fn run_gateway(host: &str, port: u16, config: Config) -> Result<()> {
    ...
    let whatsapp_app_secret: Option<Arc<str>> = std::env::var("R.A.I.N._WHATSAPP_APP_SECRET")
        .ok()
        .and_then(|secret| {
    ...
    // Priority: environment variable > config file
    let linq_signing_secret: Option<Arc<str>> = std::env::var("R.A.I.N._LINQ_SIGNING_SECRET")
        .ok()
        .and_then(|secret| {
            let secret = secret.trim();
            (!secret.is_empty()).then(|| secret.to_owned())
    ...
    let nextcloud_talk_webhook_secret: Option<Arc<str>> =
        std::env::var("R.A.I.N._NEXTCLOUD_TALK_WEBHOOK_SECRET")
            .ok()
            .and_then(|secret| {
                let secret = secret.trim();
                (!secret.is_empty()).then(|| secret.to_owned())
...
mod tests {
    ...
    fn gateway_timeout_falls_back_to_default() {
        ...
        std::env::remove_var("R.A.I.N._GATEWAY_TIMEOUT_SECS");
    ...
    async fn metrics_endpoint_renders_prometheus_output() {
        ...
        let body = response.into_body().collect().await.unwrap().to_bytes();
        let text = String::from_utf8(body.to_vec()).unwrap();
        assert!(text.contains("R.A.I.N._heartbeat_ticks_total 1"));
    }
