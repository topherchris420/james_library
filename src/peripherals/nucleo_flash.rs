//! Flash R.A.I.N. Nucleo-F401RE firmware via probe-rs.
//!
...
/// Flash R.A.I.N. Nucleo firmware. Builds from firmware/nucleo.
pub fn flash_nucleo_firmware() -> Result<()> {
    ...
    // CARGO_MANIFEST_DIR = repo root (R.A.I.N.'s Cargo.toml)
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let firmware_dir = repo_root.join("firmware").join("nucleo");
    if !firmware_dir.join("Cargo.toml").exists() {
        anyhow::bail!(
            "Nucleo firmware not found at {}. Run from R.A.I.N. repo root.",
            firmware_dir.display()
        );
    }

    println!("Building R.A.I.N. Nucleo firmware...");
    ...
    println!("R.A.I.N. Nucleo firmware flashed successfully.");
