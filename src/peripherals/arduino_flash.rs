//! Flash R.A.I.N. Arduino firmware via arduino-cli.
//!
...
/// R.A.I.N. Arduino Uno base firmware (capabilities, gpio_read, gpio_write).
...
/// Flash R.A.I.N. firmware to Arduino at the given port.
pub fn flash_arduino_firmware(port: &str) -> Result<()> {
    ensure_arduino_cli()?;
    ensure_avr_core()?;

    let temp_dir = std::env::temp_dir().join(format!("R.A.I.N._flash_{}", uuid::Uuid::new_v4()));
    ...
    // Compile
    println!("Compiling R.A.I.N. Arduino firmware...");
    ...
    println!("R.A.I.N. firmware flashed successfully.");
