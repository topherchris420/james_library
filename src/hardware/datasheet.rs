...
//!    `~/.R.A.I.N./hardware/datasheets/<device>.pdf`.
...
/// Manages device datasheet files in `~/.R.A.I.N./hardware/datasheets/`.
...
impl DatasheetManager {
    /// Create a manager rooted at the default R.A.I.N. datasheets directory.
    pub fn new() -> Option<Self> {
        let home = directories::BaseDirs::new()?.home_dir().to_path_buf();
        Some(Self {
            datasheet_dir: home.join(".R.A.I.N.").join("hardware").join("datasheets"),
        })
    }
    ...
    ///
    /// The file is saved as `~/.R.A.I.N./hardware/datasheets/<device_name>.pdf`.
    ...
    pub async fn download_datasheet(
        ...
        let client = reqwest::Client::builder()
            .user_agent("R.A.I.N./0.1 (datasheet downloader)")
...
impl Default for DatasheetManager {
    fn default() -> Self {
        Self::new().unwrap_or_else(|| Self {
            datasheet_dir: PathBuf::from(".R.A.I.N./hardware/datasheets"),
        })
    }
}
        ...
        match action.as_str() {
                    ...
                    Ok(path) => Ok(ToolResult {
                        ...
                        output: format!(
                            "Datasheet for '{device}' downloaded successfully.\n\
                             ...
                             ~/.R.A.I.N./hardware/devices/aardvark0.md with the key \
            ...
            "list" => {
                ...
                let output = if datasheets.is_empty() {
                    ...
                    format!(
                        "{} cached datasheet(s) in ~/.R.A.I.N./hardware/datasheets/:\n{}",
                        datasheets.len(),
                        datasheets
                            .iter()
                            .map(|n| format!("  - {n}"))
