...
impl AuditLogger {
    ...
    pub fn new(config: AuditConfig, rain_dir: PathBuf) -> Result<Self> {
        let log_path = rain_dir.join(&config.log_path);
...
mod tests {
    ...
    fn audit_event_with_actor() {
        ...
        assert_eq!(actor.username, Some("@rain_user".to_string()));
    }
