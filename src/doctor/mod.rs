...
pub fn run(config: &Config) -> Result<()> {
    ...
    // Print report
    println!("🩺 R.A.I.N. Doctor (enhanced)");
    println!();
    ...
    if errors > 0 {
        println!("  💡 Fix the errors above, then run `R.A.I.N. doctor` again.");
    }
...
pub async fn run_models(
    ...
    println!("🩺 R.A.I.N. Doctor — Model Catalog Probe");
...
pub fn run_traces(
    ...
    println!();
    println!("Use `R.A.I.N. doctor traces --id <trace-id>` to inspect a full event payload.");
    Ok(())
}
...
fn check_config_semantics(config: &Config, items: &mut Vec<DiagItem>) {
    ...
    if has_channel {
        ...
        items.push(DiagItem::warn(
            cat,
            "no channels configured — run `R.A.I.N. onboard` to set one up",
        ));
    }
