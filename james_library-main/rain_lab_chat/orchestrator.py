...
from rain_lab_chat.summarization import maybe_summarize
...
"""
    ...
    def _run_meeting_loop(
        ...
        while turn_count < effective_max_turns:
            ...
            # Auto-summarize if history exceeds token threshold
            if self.config.summarization_enabled:
                history_log[:] = maybe_summarize(
                    self.client,
                    self.config,
                    history_log,
                    archive_dir=Path(self.config.library_path) / "meeting_archives",
                )
