...
@Component({
  ...
  template: `
    @if (isOpen()) {
      <div class="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-center justify-center p-4" (click)="close()">
        <div class="bg-rain-bg border border-rain-green/30 w-full max-w-lg shadow-[0_0_50px_rgba(0,255,65,0.1)] relative overflow-hidden" (click)="$event.stopPropagation()">
          
          <!-- Scanline -->
          <div class="absolute top-0 left-0 w-full h-[1px] bg-rain-green/50 animate-scan pointer-events-none"></div>

          <div class="p-4 border-b border-rain-green/20 flex items-center justify-between bg-rain-panel/80">
            <h3 class="text-xs font-bold uppercase tracking-[0.2em] text-rain-red flex items-center gap-2 animate-pulse">
              <mat-icon class="text-sm">warning</mat-icon>
              // SELF-DESTRUCT IN {{ (rainService.logDestructTime().getTime() - now()) / 1000 | number:'1.0-0' }}S
            </h3>
            <button (click)="close()" class="text-rain-green/50 hover:text-rain-green transition-colors">
          ...
          <div class="p-6 space-y-6">
            ...
            <div>
              ...
              <div class="h-48 bg-black border border-rain-green/20 p-3 overflow-y-auto font-mono text-[10px] text-rain-green/80 custom-scrollbar shadow-inner">
...
export class SettingsModalComponent {
  ...
  open() {
    ...
    // Restart self-destruct timer to 5 minutes from now
