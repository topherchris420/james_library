...
description: >
  ...
  uploads, and agent memory. Requires the external `buzz` binary and a reachable
  Buzz/Nostr relay, neither of which ships with R.A.I.N.
version: 1
---

# Buzz CLI Skill

> **Ported from [block/buzz](https://github.com/block/buzz).** Upstream declared
> `name: buzz-cli` while living in a `sprout-cli/` directory; the name is
> corrected here to match the directory, which is what the skill loader keys on.

## Prerequisites (NOT satisfied by this repository)

Every command below drives the `buzz` CLI binary, which is built from
`crates/buzz-cli` in block/buzz. That crate was **not** ported into R.A.I.N. —
it transitively requires `buzz-core`, `buzz-sdk`, `buzz-persona`,
`buzz-ws-client`, `git-credential-nostr`, and `git-sign-nostr` (~43k lines of
Nostr relay stack). Until that binary is on `PATH` **and** a relay is reachable,
none of these commands will run.

Routing goes through `run_skill_tool("sprout-cli", [...])` in
`james_library/utilities/tools.py`, which resolves `$BUZZ_CLI_BIN` (default
`buzz`) and returns a structured error when it is absent.
