...
_ensure_bash() {
  ...
  if _have_cmd apk; then _run_privileged apk add --no-cache bash
    ...
    if _is_container_runtime; then
      _PACMAN_CFG="$(mktemp /tmp/R.A.I.N.-pacman.XXXXXX.conf)"
...
usage() {
  cat <<'USAGE'
R.A.I.N. installer — one-click bootstrap

Usage:
  ./install.sh [options]

The installer builds R.A.I.N., configures your provider and API key,
...
Examples:
  ...
  curl -fsSL https://R.A.I.N.labs.ai/install.sh | bash
...
Environment:
  R.A.I.N._CONTAINER_CLI     Container CLI command (default: docker; auto-fallback: podman)
  R.A.I.N._DOCKER_DATA_DIR   Host path for Docker config/workspace persistence
  R.A.I.N._DOCKER_IMAGE      Docker image tag to build/run (default: R.A.I.N.-bootstrap:local)
  R.A.I.N._API_KEY           Used when --api-key is not provided
  R.A.I.N._PROVIDER          Used when --provider is not provided (default: openrouter)
  R.A.I.N._MODEL             Used when --model is not provided
  R.A.I.N._BOOTSTRAP_MIN_RAM_MB   Minimum RAM threshold for source build preflight (default: 2048)
  R.A.I.N._BOOTSTRAP_MIN_DISK_MB  Minimum free disk threshold for source build preflight (default: 6144)
  R.A.I.N._DISABLE_ALPINE_AUTO_DEPS
...
should_attempt_prebuilt_for_resources() {
  ...
  min_ram_mb="${R.A.I.N._BOOTSTRAP_MIN_RAM_MB:-2048}"
  min_disk_mb="${R.A.I.N._BOOTSTRAP_MIN_DISK_MB:-6144}"
...
resolve_asset_url() {
  local asset_name="$1"
  local api_url="https://api.github.com/repos/R.A.I.N.-labs/R.A.I.N./releases"
...
install_prebuilt_binary() {
  ...
  asset_name="R.A.I.N.-${target}.tar.gz"
  ...
  if [[ -z "$archive_url" ]]; then
    archive_url="https://github.com/R.A.I.N.-labs/R.A.I.N./releases/latest/download/${asset_name}"
  fi

  temp_dir="$(mktemp -d -t R.A.I.N.-prebuilt-XXXXXX)"
  ...
  extracted_bin="$temp_dir/R.A.I.N."
  if [[ ! -x "$extracted_bin" ]]; then
    extracted_bin="$(find "$temp_dir" -maxdepth 2 -type f -name R.A.I.N. -perm -u+x | head -n 1 || true)"
  fi
  if [[ -z "$extracted_bin" || ! -x "$extracted_bin" ]]; then
    warn "Archive did not contain an executable R.A.I.N. binary."
    rm -rf "$temp_dir"
    return 1
  fi
  ...
  mkdir -p "$install_dir"
  install -m 0755 "$extracted_bin" "$install_dir/R.A.I.N."
  rm -rf "$temp_dir"

  step_ok "Installed pre-built binary to $install_dir/R.A.I.N."
...
run_pacman() {
  ...
  local pacman_rc=0
  pacman_cfg_tmp="$(mktemp /tmp/R.A.I.N.-pacman.XXXXXX.conf)"
...
MSG
      ...
      _xcode_test_file="$(mktemp /tmp/R.A.I.N.-xcode-check.XXXXXX.c)"
...
prompt_api_key() {
  ...
  if [[ -n "$api_key_input" ]]; then
    ...
    warn "No API key entered — you can configure it later with R.A.I.N. onboard"
    SKIP_ONBOARD=true
  fi
}
...
run_guided_installer() {
  ...
  echo
  echo -e "  ${BOLD_BLUE}${CRAB} R.A.I.N. Guided Installer${RESET}"
...
ensure_default_config_and_workspace() {
  ...
  #
  # $1 — config directory  (e.g. ~/.R.A.I.N. or $docker_data_dir/.R.A.I.N.)
  # $2 — workspace directory (e.g. ~/.R.A.I.N./workspace or $docker_data_dir/workspace)
...
# R.A.I.N. configuration — generated by install.sh
# Edit this file or run 'R.A.I.N. onboard' to reconfigure.
...
resolve_container_cli() {
  local requested_cli
  requested_cli="${R.A.I.N._CONTAINER_CLI:-docker}"
  ...
  if [[ "$requested_cli" != "docker" ]]; then
    error "Set R.A.I.N._CONTAINER_CLI to an installed Docker-compatible CLI (e.g., docker or podman)."
  else
    error "Install Docker, install podman, or set R.A.I.N._CONTAINER_CLI to an available Docker-compatible CLI."
  fi
  exit 1
}
...
run_docker_bootstrap() {
  ...
  docker_image="${R.A.I.N._DOCKER_IMAGE:-R.A.I.N.-bootstrap:local}"
  fallback_image="ghcr.io/R.A.I.N.-labs/R.A.I.N.:latest"
  if [[ "$TEMP_CLONE" == true ]]; then
    default_data_dir="$HOME/.R.A.I.N.-docker"
  else
    default_data_dir="$WORK_DIR/.R.A.I.N.-docker"
  fi
  docker_data_dir="${R.A.I.N._DOCKER_DATA_DIR:-$default_data_dir}"
  DOCKER_DATA_DIR="$docker_data_dir"

  mkdir -p "$docker_data_dir/.R.A.I.N." "$docker_data_dir/workspace"
  ...
  if [[ "$SKIP_BUILD" == false ]]; then
    ...
    if ! "$CONTAINER_CLI" image inspect "$docker_image" >/dev/null 2>&1; then
      ...
      info "Pulling official R.A.I.N. image ($fallback_image)"
  ...
  config_mount="$docker_data_dir/.R.A.I.N.:/R.A.I.N.-data/.R.A.I.N."
  workspace_mount="$docker_data_dir/workspace:/R.A.I.N.-data/workspace"
  ...
  if [[ ${#onboard_cmd[@]} -gt 0 ]]; then
    ...
    info "Docker image ready. Run R.A.I.N. onboard inside the container to configure."
  fi
...
CONTAINER_CLI="${R.A.I.N._CONTAINER_CLI:-docker}"
API_KEY="${R.A.I.N._API_KEY:-}"
PROVIDER="${R.A.I.N._PROVIDER:-openrouter}"
MODEL="${R.A.I.N._MODEL:-}"
...
if [[ "$DOCKER_MODE" == true ]]; then
  ...
  if [[ "$OS_NAME" == "Linux" && -z "${R.A.I.N._DISABLE_ALPINE_AUTO_DEPS:-}" ]] && have_cmd apk; then
    find_missing_alpine_prereqs
    if [[ ${#ALPINE_MISSING_PKGS[@]} -gt 0 && "$INSTALL_SYSTEM_DEPS" == false ]]; then
      ...
      info "Auto-enabling system dependency installation (set R.A.I.N._DISABLE_ALPINE_AUTO_DEPS=1 to disable)."
      INSTALL_SYSTEM_DEPS=true
    fi
  fi
...
if [[ ! -f "$WORK_DIR/Cargo.toml" ]]; then
  if [[ -f "$(pwd)/Cargo.toml" ]]; then
    ...
    TEMP_DIR="$(mktemp -d -t R.A.I.N.-bootstrap-XXXXXX)"
...
echo
echo -e "  ${BOLD_BLUE}${CRAB} R.A.I.N. Installer${RESET}"
...
EXISTING_VERSION=""
INSTALL_MODE="fresh"
if have_cmd R.A.I.N.; then
  EXISTING_VERSION="$(R.A.I.N. --version 2>/dev/null | awk '{print $NF}' || true)"
  INSTALL_MODE="upgrade"
elif [[ -x "$HOME/.cargo/bin/R.A.I.N." ]]; then
  EXISTING_VERSION="$("$HOME/.cargo/bin/R.A.I.N." --version 2>/dev/null | awk '{print $NF}' || true)"
  INSTALL_MODE="upgrade"
fi
...
if [[ "$INSTALL_MODE" == "upgrade" && -n "$EXISTING_VERSION" ]]; then
  step_dot "Existing R.A.I.N. installation detected, upgrading from v${EXISTING_VERSION}"
elif [[ "$INSTALL_MODE" == "upgrade" ]]; then
  step_dot "Existing R.A.I.N. installation detected, upgrading"
fi
...
if [[ "$DOCKER_MODE" == true ]]; then
  ...
  echo
  echo -e "${BOLD}Your containerized R.A.I.N. data is persisted under:${RESET}"
  echo -e "  ${DIM}$DOCKER_DATA_DIR${RESET}"
  echo
  echo -e "${BOLD}Dashboard URL:${RESET} ${BLUE}http://127.0.0.1:42617${RESET}"
  echo
  echo -e "${BOLD}Next steps:${RESET}"
  echo -e "  ${DIM}R.A.I.N. status${RESET}"
  echo -e "  ${DIM}R.A.I.N. agent -m \"Hello, R.A.I.N.!\"${RESET}"
  echo -e "  ${DIM}R.A.I.N. gateway${RESET}"
  echo
  echo -e "${BOLD}Docs:${RESET} ${BLUE}https://www.R.A.I.N.labs.ai/docs${RESET}"
  exit 0
fi
...
echo
echo -e "${BOLD_BLUE}[2/3]${RESET} ${BOLD}Installing R.A.I.N.${RESET}"
if [[ -n "$TARGET_VERSION" ]]; then
  step_dot "Installing R.A.I.N. v${TARGET_VERSION}"
fi
if [[ "$SKIP_BUILD" == false ]]; then
...
if [[ "$SKIP_INSTALL" == false ]]; then
  step_dot "Installing R.A.I.N. to cargo bin"

  # Clean up stale cargo install tracking from the old "R.A.I.N." package name
  # (renamed to "R.A.I.N.labs"). Without this, `cargo install R.A.I.N.labs` from
  # crates.io fails with "binary already exists as part of `R.A.I.N.`".
  if have_cmd cargo; then
    if [[ -f "$HOME/.cargo/.crates.toml" ]] && grep -q '^"R.A.I.N. ' "$HOME/.cargo/.crates.toml" 2>/dev/null; then
      step_dot "Removing stale cargo tracking for old 'R.A.I.N.' package name"
      cargo uninstall R.A.I.N. 2>/dev/null || true
    fi
  fi

  cargo install --path "$WORK_DIR" --force --locked
  step_ok "R.A.I.N. installed"

  # Sync binary to ~/.local/bin so PATH lookups find the fresh version
  if [[ -d "$HOME/.local/bin" ]]; then
    cp -f "$HOME/.cargo/bin/R.A.I.N." "$HOME/.local/bin/R.A.I.N." 2>/dev/null && \
...
R.A.I.N._BIN=""
if [[ -x "$HOME/.cargo/bin/R.A.I.N." ]]; then
  R.A.I.N._BIN="$HOME/.cargo/bin/R.A.I.N."
elif [[ -x "$WORK_DIR/target/release/R.A.I.N." ]]; then
  R.A.I.N._BIN="$WORK_DIR/target/release/R.A.I.N."
elif have_cmd R.A.I.N.; then
  R.A.I.N._BIN="R.A.I.N."
fi
...
if [[ "$SKIP_ONBOARD" == false && -n "$R.A.I.N._BIN" ]]; then
  if [[ -n "$API_KEY" ]]; then
    step_dot "Configuring provider: ${PROVIDER}"
    ONBOARD_CMD=("$R.A.I.N._BIN" onboard --api-key "$API_KEY" --provider "$PROVIDER")
    if [[ -n "$MODEL" ]]; then
      ONBOARD_CMD+=(--model "$MODEL")
    fi
    if "${ONBOARD_CMD[@]}" 2>/dev/null; then
      ...
      step_fail "Provider configuration failed — run R.A.I.N. onboard to retry"
    fi
  elif [[ "$PROVIDER" == "ollama" ]]; then
    step_dot "Configuring Ollama (no API key needed)"
    if "$R.A.I.N._BIN" onboard --provider ollama 2>/dev/null; then
      step_ok "Ollama configured"
    else
      step_fail "Ollama configuration failed — run R.A.I.N. onboard to retry"
    fi
  else
    ...
    if [[ -t 0 && -t 1 ]]; then
      ...
      step_dot "No API key provided — run R.A.I.N. onboard to configure"
    fi
  fi
elif [[ "$SKIP_ONBOARD" == true ]]; then
  step_dot "Skipping configuration (run R.A.I.N. onboard later)"
elif [[ -z "$R.A.I.N._BIN" ]]; then
  warn "R.A.I.N. binary not found — cannot configure provider"
fi
...
_native_config_dir="${R.A.I.N._CONFIG_DIR:-$HOME/.R.A.I.N.}"
_native_workspace_dir="${R.A.I.N._WORKSPACE:-$_native_config_dir/workspace}"
ensure_default_config_and_workspace "$_native_config_dir" "$_native_workspace_dir" "$PROVIDER"

# --- Gateway service management ---
if [[ -n "$R.A.I.N._BIN" ]]; then
  # Try to install and start the gateway service
  step_dot "Checking gateway service"
  if "$R.A.I.N._BIN" service install 2>/dev/null; then
    step_ok "Gateway service installed"
    if "$R.A.I.N._BIN" service restart 2>/dev/null; then
      ...
      for i in 1 2 3 4 5; do
        sleep 2
        if PAIR_CODE=$("$R.A.I.N._BIN" gateway get-paircode 2>/dev/null | grep -oE '[0-9]{6}'); then
          break
        fi
      done
      if [[ -n "$PAIR_CODE" ]]; then
        ...
        echo -e "  ${DIM}Run 'R.A.I.N. gateway get-paircode --new' anytime to generate a fresh code.${RESET}"
      fi
    else
      step_fail "Gateway service restart failed — re-run with R.A.I.N. service start"
    fi
  else
    step_dot "Gateway service not installed (run R.A.I.N. service install later)"
  fi

  # --- Post-install doctor check ---
  step_dot "Running doctor to validate installation"
  if "$R.A.I.N._BIN" doctor 2>/dev/null; then
    step_ok "Doctor complete"
  else
    warn "Doctor reported issues — run R.A.I.N. doctor --fix to resolve"
  fi
fi
...
INSTALLED_VERSION=""
if [[ -n "$R.A.I.N._BIN" ]]; then
  INSTALLED_VERSION="$("$R.A.I.N._BIN" --version 2>/dev/null | awk '{print $NF}' || true)"
fi

# --- Success banner ---
echo
if [[ -n "$INSTALLED_VERSION" ]]; then
  echo -e "${BOLD_BLUE}${CRAB} R.A.I.N. installed successfully (R.A.I.N. ${INSTALLED_VERSION})!${RESET}"
else
  echo -e "${BOLD_BLUE}${CRAB} R.A.I.N. installed successfully!${RESET}"
fi

if [[ -x "$HOME/.cargo/bin/R.A.I.N." ]] && ! have_cmd R.A.I.N.; then
  echo
  warn "R.A.I.N. is installed in $HOME/.cargo/bin, but that directory is not in PATH for this shell."
...
echo -e "${DIM}  Run 'R.A.I.N. gateway get-paircode' to get your pairing code.${RESET}"
