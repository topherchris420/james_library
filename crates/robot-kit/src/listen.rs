...
impl ListenTool {
    pub fn new(config: RobotConfig) -> Self {
        let recordings_dir = directories::UserDirs::new()
            .map(|d| d.home_dir().join(".R.A.I.N./recordings"))
            .unwrap_or_else(|| PathBuf::from("/tmp/R.A.I.N._recordings"));
    ...
    async fn transcribe(&self, audio_path: &Path) -> Result<String> {
        ...
        // whisper.cpp model path (typically in ~/.R.A.I.N./models/)
        let model_path = directories::UserDirs::new()
            .map(|d| {
                d.home_dir()
                    .join(format!(".R.A.I.N./models/ggml-{}.bin", model))
            })
            .unwrap_or_else(|| {
