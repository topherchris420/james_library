...
mod tests {
    ...
    fn effort_table_fixture_matches_rust_implementation() {
        // Upstream reads this from block/buzz's `desktop/` TypeScript frontend,
        // where the fixture is the shared contract between the Rust effort table
        // and the agent-settings UI. R.A.I.N. has no such frontend, so the
        // fixture is vendored into the crate: the test keeps its value as a
        // regression lock on the Rust table, minus the monorepo path escape.
        let fixture_json = include_str!("../tests/fixtures/effortTable.fixture.json");
