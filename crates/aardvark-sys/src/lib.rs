...
//!
//! 1. `R.A.I.N._AARDVARK_LIB` environment variable (full path to `aardvark.so`)
...
//! # Safety
//!
//! This crate is the **only** place in R.A.I.N. where `unsafe` is permitted.
    ...
    AARDVARK_LIB
        .get_or_init(|| {
            let candidates: Vec<PathBuf> = vec![
                ...
                std::env::var("R.A.I.N._AARDVARK_LIB")
                    .ok()
                    .map(PathBuf::from)
                    .unwrap_or_default(),
            ...
            if !tried_any {
                eprintln!("[aardvark-sys] no library candidates found; set R.A.I.N._AARDVARK_LIB or place aardvark.so next to the binary");
            }
            None
        })
        .as_ref()
...
pub enum AardvarkError {
    ...
    #[error("aardvark.so not found — set R.A.I.N._AARDVARK_LIB or place it next to the binary")]
    LibraryNotFound,
}
