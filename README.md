...
### Identity Formats (OpenClaw + AIEOS)

ZeroClaw already supports identity-agnostic personas in two formats:

1. **OpenClaw (default, workspace markdown)**
   - `IDENTITY.md` (agent identity)
   - `SOUL.md` (values/personality)
   - `USER.md` (who the agent helps)
   - `AGENTS.md` (behavior rules)
2. **AIEOS v1.1 JSON**
   - Import/export portable identity payloads
   - Keep behavioral intent portable across OpenAI-compatible backends

Enable AIEOS in your `config.toml`:
...
If `format = "openclaw"` (or omitted), ZeroClaw uses workspace identity markdown files by default.
