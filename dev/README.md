# R.A.I.N. Development Environment

A fully containerized development sandbox for R.A.I.N. agents. This environment allows you to develop, test, and debug the agent in isolation without modifying your host system.
...
### 2. Enter Agent Container (`R.A.I.N.-dev`)

```bash
./dev/cli.sh agent
```

Use this to run `R.A.I.N.` CLI commands manually, debug the binary, or check logs internally.
...
- **Agent**: `/R.A.I.N.-data/workspace`
- **Sandbox**: `/home/developer/workspace`

Files created by the agent are visible to the sandbox user, and vice versa.

The agent configuration lives in `target/.R.A.I.N.` (mounted to `/R.A.I.N.-data/.R.A.I.N.`), so settings persist across container rebuilds.
...
**Note:** This removes `target/.R.A.I.N.` (config/DB) but leaves the `playground/` directory intact. To fully wipe everything, manually delete `playground/`.
...
- Rust compilation, tests, and audit/deny tools run in `R.A.I.N.-local-ci` container.
...
- The root `Dockerfile` also caches Rust `target/` (`id=R.A.I.N.-target`) to speed repeat local image builds.
