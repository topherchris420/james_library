...
ASSET_URL="https://github.com/R.A.I.N.-labs/R.A.I.N./releases/download/${TAG}/${ASSET_NAME}"
TEMP_DIR="$(mktemp -d -t R.A.I.N.-termux-test-XXXXXX)"
...
# --- Test 5: Contains R.A.I.N. binary ---
info "Checking archive contents"
CONTENTS=$(tar -tzf "$TEMP_DIR/$ASSET_NAME")
if echo "$CONTENTS" | grep -q "^R.A.I.N.$"; then
  pass "Archive contains 'R.A.I.N.' binary"
else
  fail "Archive does not contain 'R.A.I.N.' binary"
  echo "Contents:"
  echo "$CONTENTS" | sed 's/^/  /'
fi
...
CHECKSUMS_URL="https://github.com/R.A.I.N.-labs/R.A.I.N./releases/download/${TAG}/SHA256SUMS"
