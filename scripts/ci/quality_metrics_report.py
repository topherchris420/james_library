    ...
    output_md.write_text(
        "\n".join(
            [
                ...
                f"| Flaky test rate | {result.flaky_test_rate_pct}% ({result.flaky_tests}/{result.total_tests_in_run1}) |",
                f"| Mean PR size (LOC, {args.days}d) | {result.mean_pr_size_loc} |",
                f"| Critical-path test coverage | {result.critical_path_coverage_pct}% ({result.critical_path_executed}/{result.critical_path_discovered}) |",
                "",
