#!/usr/bin/env bash
# deploy-rpi.sh — cross-compile R.A.I.N. for Raspberry Pi and deploy via SSH.
#
...
#   RPI_DIR         — remote deployment dir           (default: /home/$RPI_USER/R.A.I.N.)
...
RPI_DIR="${RPI_DIR:-/home/${RPI_USER}/R.A.I.N.}"
TARGET="aarch64-unknown-linux-gnu"
FEATURES="hardware,peripheral-rpi"
BINARY="target/${TARGET}/release/R.A.I.N."
...
echo "==> Building R.A.I.N. for Raspberry Pi (${TARGET})"
...
echo ""
echo "==> Stopping R.A.I.N. service (if running)"
# shellcheck disable=SC2029
${SSH_CMD} ${SSH_OPTS} "${RPI_USER}@${RPI_HOST}" \
  "sudo systemctl stop R.A.I.N. 2>/dev/null || true"
...
# ── 4. Deploy binary ──────────────────────────────────────────────────────────
echo ""
echo "==> Deploying binary to ${RPI_USER}@${RPI_HOST}:${RPI_DIR}/R.A.I.N."
${SCP_CMD} ${SCP_OPTS} "${BINARY}" "${RPI_USER}@${RPI_HOST}:${RPI_DIR}/R.A.I.N."
...
# ── 5. Deploy config ─────────────────────────────────────────────────────────
CONFIG_DEST="/home/${RPI_USER}/.R.A.I.N./config.toml"
echo ""
echo "==> Deploying config to ${CONFIG_DEST}"
# shellcheck disable=SC2029
${SSH_CMD} ${SSH_OPTS} "${RPI_USER}@${RPI_HOST}" "mkdir -p /home/${RPI_USER}/.R.A.I.N."
...
SERVICE_DEST="/etc/systemd/system/R.A.I.N..service"
echo ""
echo "==> Installing systemd service (requires sudo on the Pi)"
${SCP_CMD} ${SCP_OPTS} "scripts/R.A.I.N..service" "${RPI_USER}@${RPI_HOST}:/tmp/R.A.I.N..service"
# shellcheck disable=SC2029
${SSH_CMD} ${SSH_OPTS} "${RPI_USER}@${RPI_HOST}" \
  "sudo mv /tmp/R.A.I.N..service ${SERVICE_DEST} && \
   sudo systemctl daemon-reload && \
   sudo systemctl enable R.A.I.N. && \
   sudo systemctl restart R.A.I.N. && \
   sudo systemctl status R.A.I.N. --no-pager || true"
...
# ── 8. Reset ACT LED trigger so R.A.I.N. can control it ──────────────────────
echo ""
...
echo ""
echo "    R.A.I.N. is running at http://${RPI_HOST}:8080"
echo "    POST /api/chat  — chat with the agent"
echo "    GET  /health    — health check"
echo ""
echo "    To check logs: ssh ${RPI_USER}@${RPI_HOST} 'journalctl -u R.A.I.N. -f'"
